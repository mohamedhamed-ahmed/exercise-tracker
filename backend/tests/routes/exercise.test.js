...
describe('Exercise Endpoints', () => {
  ...
  describe('POST Endpoint', () => {
    it('should successfully create a new exercise', async () => {
      const res = await request(app)
        .post(`/api/users/${users[0].id}/exercises`)
        .send({
  ...
  describe('DELETE Endpoint', () => {
    ...
    it('should successfully delete an exercise', async () => {
      const res = await request(app)
        .delete(`/api/users/${users[0].id}/exercises/${exerciseRes.body.id}`)
        .send({});
      expect(res.statusCode).toEqual(200);
    });
    it('should return 404 if user is not found', async () => {
      const res = await request(app)
        .delete(`/api/users/9999/exercises/${exerciseRes.body.id}`)
        .send({});
      expect(res.statusCode).toEqual(404);
    });
  });
